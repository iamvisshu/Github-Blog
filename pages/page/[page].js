...
export default function PostListPage({ posts, search = "", numPages, currentPage }) {
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row gap-8 p-4 md:p-8 bg-gray-50 dark:bg-gray-900 min-h-screen overflow-x-hidden">
        ...
        <aside className="w-full md:w-80 bg-white dark:bg-gray-800 rounded-2xl shadow p-6 hidden md:block">
            {/* The actual sidebar content from index.js would go here,
                or you can import a Sidebar component for consistency. */}
            <p className="dark:text-white">Sidebar Content Placeholder</p>
        </aside>
        ...
        <main className="flex-1 space-y-8">
          {filteredPosts.length === 0 && search.length > 0 ? (
            ...
            // CHANGE: Use PostCard component here
...
// Get paths for all numbered pages (e.g., /page/2, /page/3)
...
export async function getStaticProps({ params }) {
  ...
  // 3. Slice the posts to only show the posts for the current page
